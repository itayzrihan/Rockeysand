...
#if defined(NDEBUG)
# error "Rockeysand cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xed60ee1a764fc8ec5b6c4b9e0eca389cf511c95be6b36e7898243e234aa7132e");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Rockeysand: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Rockeysand Signed Message:\n";
...
{
    // Rockeysand: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Rockeysand: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Rockeysand: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Rockeysand: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Rockeysand: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Rockeysand: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xacb6db9e047d71581832cd0db1fef0fb5ffb0eb31e4e390053246a855e3d1c5e");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "4/4/21 The day sand turned into Pyramid";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("040e5078d8ef170c0acdf37a50df5a1603c3b4b6e8a116c6ff866da2c062ff5e5fccf7c3dde10d7c382c493039930a0c7b50b9e111c623cc50670afe509c68d860") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1617547256;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084727923;

        if (fTestNet)
        {
            block.nTime    = 1617547265;
            block.nNonce   = 386022529;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x4bf1a4b50a137a69b8ce279d93521bce27fe7adf81cb2b0e7f37fc712f30edc5"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc6, 0xb9, 0xdd }; // Rockeysand: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("RockeysandMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("RockeysandMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static RockeysandMiner(CWallet *pwallet)
{
    printf("RockeysandMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("rockeysand-miner");
    ...
    try { loop {
        ...
        printf("Running RockeysandMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("RockeysandMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&RockeysandMiner, pwallet));
}
